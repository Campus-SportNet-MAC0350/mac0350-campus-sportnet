...
export const DisplayPublication = () => {
    const listPosts = publications.map(publication => 
        <Post publicationType={publication.publicationType} username={publication.username} profileImage={publication.profileImage} postText={publication.postText} imageUrl={publication.imageUrl} eventData={publication.eventData} eventTime={publication.eventTime} confirmedUsers={publication.confirmedUsers}/>
    )
    return(
        <div className="feed">
            {listPosts}
